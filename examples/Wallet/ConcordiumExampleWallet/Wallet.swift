...
public class Wallet {
    ...
    private let walletProxy: WalletProxyEndpoints
    ...
    public func refresh() async throws {
        let ips = try await walletProxy.getIdentityProviders.response(session: URLSession.shared)
    ...
    public func prepareRecoverIdentity(providerID: IdentityProviderID, index: IdentityIndex) throws -> IdentityRecoverRequest {
        guard let provider = identityProviders[providerID] else {
            throw WalletError.identityProviderNotFound(providerID)
        }
        return try identityRequestURLBuilder.recoveryRequestToFetch(
